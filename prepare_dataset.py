...
class my_dataset_contrastive(Dataset):
    ...
    def __init__(self, basepath_to_data, dataset_name, phase, inference, fractions, acquired_items,
                 modalities=['ecg', 'ppg'], task='contrastive', input_perturbed=False, perturbation='Gaussian',
                 leads='ii', heads='single', cl_scenario=None, class_pair='', trial='CMC', nviews=1):
        ...
        fraction = fractions[
            'fraction']  # needs to be a list when dealing with 'query' or inference = True for CL scenario
        ...
        if task == 'self-supervised':
            ...
            self.name = '-'.join(
                (dataset_name, modalities[0], str(fraction), leads, class_pair))  # name for different tasks
            if phase == 'train':
                if inference == False:
                    inputs, outputs, pids = self.expand_labelled_data(input_array, output_array, pid_array, fraction,
                                                                      labelled_fraction, unlabelled_fraction,
                    ...
                    inputs, outputs, modality_array, keep_indices = self.retrieve_modified_unlabelled_data(input_array,
                                                                                                           output_array,
                                                                                                           fraction,
                                                                                                           unlabelled_fraction,
                                                                                                           acquired_indices)
                ...
                inputs, outputs, pids = self.retrieve_val_data(input_array, output_array, pid_array, phase, fraction,
    ...
    def load_raw_inputs_and_outputs(self, dataset_name, leads='i'):
        ...
        if dataset_name == 'bidmc':
            ...
            leads_name = leads
            path = os.path.join(basepath, 'PhysioNetChallenge2020_Training_CPSC', 'Training_WFDB', 'patient_data',
                                self.task, 'leads_%s' % leads_name)
            extension = ''
    ...
    def offset_outputs(self, dataset_name, outputs,
                       t=0):  # t tells you which class pair you are on now (used rarely and only for MTL)
    ...
    def retrieve_buffered_data(self, buffer_indices_dict, fraction, labelled_fraction):
        ...
        for task_name, indices in buffer_indices_dict.items():
            ...
            input_array, output_array = self.retrieve_labelled_data(input_array, output_array, fraction,
                                                                    labelled_fraction, dataset_name=dataset)
    ...
    def expand_labelled_data_with_buffer(self, input_array, output_array, buffer_indices_dict, fraction,
                                         labelled_fraction):
        """ function arguments are raw inputs and outputs """
        input_buffer, output_buffer, task_indices_buffer, dataset_buffer = self.retrieve_buffered_data(
            buffer_indices_dict, fraction, labelled_fraction)
        if self.cl_scenario == 'Class-IL':
            self.class_pair = '-'.join(self.name.split('-')[-2:])  # b/c e.g. '0-1' you need last two
        elif self.cl_scenario == 'Time-IL':
            self.class_pair = self.name.split('-')[-1]
        input_array, output_array = self.retrieve_labelled_data(input_array, output_array, fraction, labelled_fraction,
    ...
    def retrieve_val_data(self, input_array, output_array, pid_array, phase, fraction, labelled_fraction=1,
                          dataset_name=''):  # ,modalities=['ecg','ppg']):
        frame_array = []
        label_array = []
        pids = []
    ...
    def retrieve_unlabelled_data(self, input_array, output_array, fraction,
                                 unlabelled_fraction):  # ,modalities=['ecg','ppg']):
        phase = 'train'
        frame_array = []
        label_array = []
        modality_array = []
    ...
    def retrieve_modified_unlabelled_data(self, input_array, output_array, fraction, unlabelled_fraction,
                                          acquired_indices):
        inputs, outputs, modality_array = self.retrieve_unlabelled_data(input_array, output_array, fraction,
                                                                        unlabelled_fraction)
        inputs, outputs, modality_array, keep_indices = self.remove_acquired_data(inputs, outputs, modality_array,
                                                                                  acquired_indices)
        return inputs, outputs, modality_array, keep_indices

    def retrieve_labelled_data(self, input_array, output_array, pid_array, fraction, labelled_fraction,
                               dataset_name=''):  # ,modalities=['ecg','ppg']):
        phase = 'train'
        frame_array = []
        label_array = []
    ...
    def retrieve_multi_task_train_data(self):
        ...
        output_array = []
        pids = []
        for t, (dataset_name, fraction, leads, class_pair) in enumerate(
                zip(self.dataset_name, self.fraction, self.leads, all_class_pairs)):  # should be an iterable list
            ...
            current_input, current_output, pid = self.retrieve_labelled_data(current_input, current_output, pid,
                                                                             fraction, self.labelled_fraction,
                                                                             dataset_name=dataset_name)
    ...
    def retrieve_multi_task_val_data(self, phase):
        ...
        output_array = []
        pids = []
        for t, (dataset_name, modalities, fraction, leads, class_pair) in enumerate(
                zip(self.dataset_name, all_modalities, self.fraction, self.leads,
                    all_class_pairs)):  # should be an iterable list
            current_input, current_output, pid = self.load_raw_inputs_and_outputs(dataset_name, leads=leads)
            self.class_pair = class_pair
            self.modalities = modalities
            current_input, current_output, pid = self.retrieve_val_data(current_input, current_output, pid, phase,
                                                                        fraction,
                                                                        dataset_name=dataset_name)  # ,labelled_fraction=1)
            # current_output = self.offset_outputs(dataset_name,current_output,t)
            input_array.append(current_input)
            output_array.append(current_output)
            pids.append(pid)
    ...
    def expand_labelled_data(self, input_array, output_array, pid_array, fraction, labelled_fraction,
                             unlabelled_fraction, acquired_indices, acquired_labels):
        inputs, outputs, pids = self.retrieve_labelled_data(input_array, output_array, pid_array, fraction,
                                                            labelled_fraction, self.dataset_name)
        ...
        if len(acquired_indices) > 0:
            acquired_inputs, acquired_outputs, acquired_modalities = self.acquire_unlabelled_samples(input_array,
                                                                                                     output_array,
                                                                                                     fraction,
                                                                                                     unlabelled_fraction,
                                                                                                     acquired_indices)
    ...
    def normalize_frame(self, frame):
        if self.dataset_name not in ['cardiology', 'physionet2017',
                                     'physionet2016']:  # or self.dataset_name != 'physionet2017':# or self.dataset_name != 'cipa':
    ...
    def __getitem__(self, index):
        ...
        if self.trial == 'CMSC':
            ...
            frame_views = torch.empty(1, 2500,
                                      self.nviews * 2)  # self.nviews) #nviews = nleads in this case (1x2500x12*nsegments)
